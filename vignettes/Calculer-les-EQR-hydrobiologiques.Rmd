...
```{r}
donnees<-import_hubeau_indices_hbio(liste_stations = c("04207400", "04216000"))
str(donnees)
```
...
```{r}
str(data(base_ref_eqr))

```

## Objet de classe seuil_eqr_hydrobio

On peut convertir cette table (ou toute table présentant les mêmes noms de colonnes) en objet de classe S4 : seuil_eqr_hydrobio.
Ces objets relient une typologie de station et un paramètre (IBD ou IBMR) aux valeurs de référence qui permettent le calcul de l'EQR.
