...
```{r}
donnees<-data.frame(Parametre=rep(letters[1:5], 100), RsAna=sample(0.1:10, 500, replace=TRUE), LqAna=c(0.5,1,2,6))
...
seuil<-list(CdParametre=NA, Nom=c(paste0(intToUtf8(8804), "1µg/L"),"1 à 5 µg/L", ">5 µg/L"), Bornes=c(0,1,5,+Inf), include.lowest=T)
seuils0<-makeSeuils(type_seuil="DCE", CdParametre = "1340")


donnees$categorie<-cut(donnees$RsAna, seuil$Bornes, labels=seuil$Nom, include.lowest = seuil$include.lowest, ordered_result = T)
donnees0<-donnees%>%group_by(Parametre, categorie, CdRqAna)%>%dplyr::summarise(nb = n())%>%mutate(categorie=fct_rev(categorie))

donnees0$etiquette<-paste0(ifelse(donnees0$CdRqAna=="1", paste0(donnees0$categorie), paste0("<LQ avec LQ ", ifelse(substr(donnees0$categorie,1,1)%in%c("<",">", "=", intToUtf8(8804), intToUtf8(2265)), "", "de "), donnees0$categorie)))

levels(donnees$categorie)%>%as.character
donnees0$etiquette<-factor(donnees0$etiquette)

summary(donnees0)

ggplot(donnees0, aes(x=nb, y=Parametre, fill=categorie, alpha=CdRqAna))+ 
  geom_bar(position="fill", stat="identity") + scale_alpha_manual(values=c(1,0.20))
