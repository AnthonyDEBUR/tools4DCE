...
filtre_donnees_extremes <- function(x, ymin, ymax){
  ...
  if(!is.na(ymin)){x[x<ymin]<-ymin}
  if(!is.na(ymin)){x[x>ymax]<-ymax}
  if(is.na(ymin) & is.na(ymax)){return("Au moins une valeur entre ymin et ymax doit etre renseignee")}else{return(x)}
