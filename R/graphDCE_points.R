...
#' @export
#'
graphDCE_points<-function(data,col_dates="DatePrel", col_valeurs="RsAna", col_LQ="LqAna",
                         seuils=NULL, affiche_legende=T, nom_legende=NULL, titre=NULL,taille_titre=12,
                         unite=NULL,bilan_annuel=FALSE,
                         xmini=NULL, xmaxi=NULL, ymini=NULL, ymaxi=NULL,
                         auto_ymaxi=TRUE,
                         lignes=NULL, echelleLog=FALSE, taille_legende=12, liaison=T, affiche_LQ=F,
                         separ_stations=NULL,  alpha=0.8)
{
  ...
  if(nrow(data1)>0) # on ne traite les données que si le tableau de données n'est pas vide
  {
    ...
    if(bilan_annuel)
    {
      ...
      if(is.null(xmini)){xmini<-as.POSIXct(strptime(paste0(format((min(data1$DatePrel, na.rm=T)-1),"%Y"),"-01-01 00:00:00"), "%Y-%m-%d %H:%M:%S"), tz="Europe/Paris")}
      else {xmini<-as.POSIXct(strptime(paste0(xmini,"-01-01 00:00:00"), "%Y-%m-%d %H:%M:%S"), tz="Europe/Paris")}
      if(is.null(xmaxi)){xmaxi<-as.POSIXct(strptime(paste0(format((max(data1$DatePrel, na.rm=T)+1),"%Y"),"-12-31 23:59:59"), "%Y-%m-%d %H:%M:%S"), tz="Europe/Paris")}
      else {xmaxi<-as.POSIXct(strptime(paste0(xmaxi,"-12-31 23:59:59"), "%Y-%m-%d %H:%M:%S"), tz="Europe/Paris")}
    ...
    {
      # paramètrage xmini - maxi pour les échelles temporelles
      if(is.null(xmini)){xmini<-min(data1$DatePrel, na.rm=T)}
      else {xmini<-as.POSIXct(strptime(paste0(xmini,"-01-01 00:00:00"), "%Y-%m-%d %H:%M:%S"), tz="Europe/Paris")}
      if(is.null(xmaxi)){xmaxi<-max(data1$DatePrel, na.rm=T)}
      else {xmaxi<-as.POSIXct(strptime(paste0(xmaxi,"-12-31 23:59:59"), "%Y-%m-%d %H:%M:%S"), tz="Europe/Paris")}
      # on ne conserve que les données entre les dates xmini et xmaxi
      data1<-subset(data1, DatePrel>=xmini & DatePrel<=xmaxi)
    ...
    else if(duree_jours%/%365<=29){break_date_max<-"5 years"
    dateformat<-"%b%Y"}
    # durée >30 ans
    else {break_date_max<-"10 years"
    dateformat<-"%b%Y"}
    ...
    rangedata<-abs(max(data1$RsAna, na.rm=T)-min(data1$RsAna, na.rm=T))
    ...
    if(rangedata==0 & !is.null(seuils1)){rangedata<-abs(max(jitter(data1$RsAna), na.rm=T)-min(data1$RsAna, na.rm=T))} # si toutes les valeurs de données sont égales alors on calcul un range autour des valeurs mesurées en ajoutant un bruit artificiel dans le calcul du range (jitter)
    ...
    if(is.null(ymaxi)){seuils1minmax<-c(seuils1minmax[seuils1minmax<=max_data], max_data)}
    ...
    if(echelleLog & length(seuils1minmax[seuils1minmax<=0])>0){seuils1minmax<-seuils1minmax[seuils1minmax>0]}
    ...
    if(!is.null(seuils1)){
      ...
      if(nrow(seuils1[seuils1$SEUILMIN<min(seuils1minmax, na.rm=T),])>0){seuils1[seuils1$SEUILMIN<min(seuils1minmax, na.rm=T),]$SEUILMIN<-min(seuils1minmax, na.rm=T)}
      if(nrow(seuils1[seuils1$SEUILMAX<min(seuils1minmax, na.rm=T),])>0){seuils1[seuils1$SEUILMAX<min(seuils1minmax, na.rm=T),]$SEUILMAX<-min(seuils1minmax, na.rm=T)}
      if(nrow(seuils1[seuils1$SEUILMIN>max(seuils1minmax, na.rm=T),])>0){seuils1[seuils1$SEUILMIN>max(seuils1minmax, na.rm=T),]$SEUILMIN<-max(seuils1minmax, na.rm=T)}
      if(nrow(seuils1[seuils1$SEUILMAX>max(seuils1minmax, na.rm=T),])>0){seuils1[seuils1$SEUILMAX>max(seuils1minmax, na.rm=T),]$SEUILMAX<-max(seuils1minmax, na.rm=T)}


      # préparation de la commande sur les couleurs des rectangles
      couleurs<-""
      for(k in 1:nrow(seuils1))
      {couleurs<-paste0(couleurs,'"', seuils1$CLASSE[k],
    ...
    data1$depassementSUP<-ifelse(data1$RsAna>max(seuils1minmax, na.rm=T),max(seuils1minmax[seuils1minmax<Inf], na.rm=T) ,NA)
    data1$depassementINF<-ifelse(data1$RsAna<min(seuils1minmax, na.rm=T),min(seuils1minmax[seuils1minmax>-Inf], na.rm=T),NA)
    if(any(!is.na(data1$depassementSUP))){depassSUP<-subset(data1, !is.na(depassementSUP))
    data1[which(!is.na(data1$depassementSUP)),]$RsAna<-max(seuils1minmax[seuils1minmax<Inf], na.rm=T)
    }
    if(any(!is.na(data1$depassementINF))){depassINF<-subset(data1, !is.na(depassementINF))
    data1[which(!is.na(data1$depassementINF)),]$RsAna<-min(seuils1minmax[seuils1minmax>-Inf], na.rm=T)
    }
    ...
    if(is.null(nom_legende))
    {
      ...
      if(!is.null(seuils)){nom_legende<-seuils[[1]]@nom_seuil}else{nom_legende<-""}
     }
    ...
    if(is.null(unite)){
      if(!is.null(seuils)){unite<-tools4DCE::unites_sandre[tools4DCE::unites_sandre$`Code de l'unité de référence`==seuils[[1]]@code_unite,]$`Symbole de l'unité de référence`[1]}
      else {unite<-""}
    }
    ...
    # si seuils est également null alors on affecte une étiquette vide à unité
    if(is.null(titre)){
      if(!is.null(seuils)){titre<-seuils[[1]]@nom_parametre}
      }
    ...
    # ajout des couleurs
    if(!is.null(seuils1))
    {graph1<-graph1 + geom_rect(data=seuils1, aes(xmin=xmini, xmax=xmaxi,ymin=SEUILMIN, ymax=SEUILMAX, fill=CLASSE),alpha=alpha, show.legend =T)
    graph1<-graph1 + scale_fill_manual(name=nom_legende,drop=FALSE,values=eval(parse(text=couleurs)))
    }

    # échelle log ou pas
    if(echelleLog){graph1<-graph1 + scale_y_log10(breaks=seuils1minmax, expand = c(0, 0), limits=c(min(seuils1minmax[seuils1minmax>-Inf]),max(seuils1minmax[seuils1minmax<Inf])))}
    if(!echelleLog){graph1<-graph1 + scale_y_continuous(breaks=seuils1minmax, expand = c(0, 0), limits=c(min(seuils1minmax[seuils1minmax>-Inf]),max(seuils1minmax[seuils1minmax<Inf])))}

    # échelle temporelle
    graph1<-graph1 + scale_x_datetime(labels=date_format(dateformat, tz="Europe/Paris"),date_breaks = break_date_max, date_minor_breaks = break_date_min, limits=c(xmini,xmaxi), expand = c(0, 0))

    # ajout de ligne entre les points si option retenue
    if(liaison){
      if(is.null(separ_stations)){graph1<-graph1 + geom_line(data=data1, aes(x=DatePrel, y=RsAna), linetype="dashed")}
      else {eval(parse(text=paste0('graph1<-graph1 + geom_line(data=data1, aes(x=DatePrel, y=RsAna, linetype = ',separ_stations,'))')))}
    }
    ...
    if(affiche_LQ)
    {
      ...
      graph1<-graph1 + geom_ribbon(data=data1, aes(x=DatePrel, ymin=min(seuils1minmax[seuils1minmax>-Inf]), ymax=LqAna),fill="grey30",
                                   stat = "identity", position = "identity", show.legend = F)
    }
    ...
    {
      eval(parse(text=paste0("graph1<-graph1 + geom_point(data=data1, aes(x=DatePrel, y=RsAna, shape = ",separ_stations,"), size=3)")))
    }

    # ajout du titre et des noms d'axes
    if(!is.null(titre)){graph1<-graph1 + ggtitle(titre) + theme(plot.title = element_text(size=taille_titre))}
    graph1<-graph1 +xlab('') +ylab(unite)
    # ajout des lignes au niveau des seuils1
    if(length(lignes)>0){graph1<-graph1+ geom_hline(yintercept=lignes,linetype="dashed")}

    # ajout des indications de valeurs hors plage de données
    if(any(!is.na(data1$depassementSUP))){graph1<-graph1 + geom_label(data=depassSUP, aes(x=DatePrel, y=max(seuils1minmax[seuils1minmax<Inf]), label=RsAna), fill=depassSUP$couleur_pt, size=2.7, vjust="top")}
    if(any(!is.na(data1$depassementINF))){graph1<-graph1 + geom_label(data=depassINF, aes(x=DatePrel, y=min(seuils1minmax[seuils1minmax>-Inf]), label=RsAna), fill=depassINF$couleur_pt, size=2.7, vjust="bottom")}

    # gestion de l'échelle et des indications sur les axes
    graph1<-graph1 + theme_light()+
      theme(legend.position = ifelse(affiche_legende,'right','none'),
