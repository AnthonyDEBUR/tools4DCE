...
#' @export
#'
graphDCE_bar<-function(data,col_annee="annee", col_valeurs="RsAna", ymaxi=NULL,auto_ymaxi=TRUE,
                          seuils=NULL, unite=NULL, titre=NULL,taille_titre=12, affiche_legende=T,
                       nom_legende=NULL,taille_legende=12, lignes=NULL, alpha=0.8)
{
...
if(!is.null(seuils1)){graph1<- graph1+geom_rect(data=seuils1, aes(xmin=min(Xdo), xmax=max(Xdo),ymin=SEUILMIN, ymax=SEUILMAX, fill=CLASSE),alpha=alpha, show.legend =T)
...
graph1<-graph1 + scale_y_continuous(breaks=seuils1minmax, expand = c(0, 0), limits=c(min(seuils1minmax[seuils1minmax>-Inf]),max(seuils1minmax[seuils1minmax<Inf])))
...
# ajout des barres
  graph1<-graph1+geom_bar(data=data1, aes(x=ANNEE, y=RsAna), stat="identity",colour="black", alpha=0.2, width=1)


  # ajout des indications de valeurs hors plage de données
  if(any(!is.na(data1$depassementSUP))){graph1<-graph1 + geom_label(data=depassSUP, aes(x=ANNEE, y=max(seuils1minmax[seuils1minmax<Inf]), label=RsAna), fill=depassSUP$couleur_pt, size=2.7, vjust="top")}
  if(any(!is.na(data1$depassementINF))){graph1<-graph1 + geom_label(data=depassINF, aes(x=ANNEE, y=min(seuils1minmax[seuils1minmax>-Inf]), label=RsAna), fill=depassINF$couleur_pt, size=2.7, vjust="bottom")}
