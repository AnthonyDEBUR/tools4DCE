...
=======
#' @examples import_QESU_PHY_v3(x)
>>>>>>> 740885c77eb65c7c3660dc68437503eed8419c59
#' @export
import_QESU_PHY_v3 <- function(x) {
  ...
  if (xml_ns(file)[1] != "http://xml.sandre.eaufrance.fr/scenario/quesu/3") {
    stop("le scénario du fichier n'est pas de type QUESU_PHY_V3")
  }
...
=======
    ...
    if (grepl("Prelevement", divs[i])) {
      valeurs <- divs[i] %>% xml_contents()%>% xml_contents()
      ...
      CdRdd1 <-
        valeurs[grep("<Rsx>", valeurs)] %>% xml_contents()
      if (length(CdRdd1) > 0) {
        for (k in 1:length(CdRdd1)) {
          CdRdd0 <- CdRdd1[k] %>% xml_contents()  %>% as.character
          ifelse(k == 1,
                 CdRdd <- CdRdd0,
      ...
      # CdMethode <-
      #   valeurs[grep("<Methode>", valeurs)]
      # CdMethode <- CdMethode[1] %>% xml_contents()
      # CdMethode <-
      #   CdMethode[grep("<CdMethode>", CdMethode)]
      ...
      CdZoneVerticaleProspectee <-
        valeurs[grep("<ZoneVerticaleProspectee>", valeurs)] %>% xml_contents()
      CdZoneVerticaleProspectee <-
      ...
      ProfondeurPrel <-
        valeurs[grep("<ProfondeurPrelevement>", valeurs)] %>% xml_contents()
      ProfondeurPrel <-
      ...
      CdFinalitePrel <-
        valeurs[grep("<FinalitePrel>", valeurs)] %>% xml_contents()
...
#j<-Cond_Env[1]
      output <- lapply(Cond_Env, function(j) {
        ...
        CdUniteMesure <-
          valeurs2[grep("<UniteMesure>", valeurs2)] %>% xml_contents() %>% xml_contents()
        CdUniteMesure <- CdUniteMesure[1] %>% as.character
        if (length(CdUniteMesure) == 0) {
          CdUniteMesure <- NA
        }
        ...
        CdProducteur <-
          valeurs2[grep("<Responsable", valeurs2)] %>% xml_contents() %>% xml_contents()
        CdProducteur <- CdProducteur[1] %>% as.character
        if (length(CdProducteur) == 0) {
          CdProducteur <- ""
        }
